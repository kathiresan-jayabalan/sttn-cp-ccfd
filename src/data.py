...
def feature_matrix(df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray, list[str]]:
...
) -> Tuple[Tuple[np.ndarray, np.ndarray], Tuple[np.ndarray, np.ndarray], Tuple[np.ndarray, np.ndarray], MinMaxScaler, list[str]]:
...
def build_labeled_windows(
    ...
    """Create partition-local windows and label each by its final row."""
    if X.ndim != 2:
    ...
    windows = np.stack(
        [X[i : i + seq_len] for i in range(len(X) - seq_len + 1)], axis=0
    ).astype(np.float32)
